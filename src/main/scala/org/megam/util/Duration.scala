 ...
 *
 * {{{
 * import org.megam.conversions.time._
 *
...
sealed class Duration private[util] (protected val nanos: Long) extends {
   ...
   * - is easy to read
   * - can be read back in if org.megam.conversions.time._ is imported
   *
   * An example:
   *
   * org.megam.util.Duration(9999999, java.util.concurrent.TimeUnit.MICROSECONDS)
   * res0: org.megam.util.Duration = 9.seconds+999.milliseconds+999.microseconds
   */
