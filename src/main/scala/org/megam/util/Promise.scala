...
import org.megam.concurrent.Scheduler
...
object Promise {
  ...
  sealed trait Responder[A] extends Future[A] {
     ...
     * {{org.megam.util.Monitor}} for details.
     */
...
/**
 * A writeable [[org.megam.util.Future]] that supports merging.
 * Callbacks (responders) of Promises are scheduled with
 * [[org.megam.concurrent.Scheduler]].
 *
 ...
 * [[org.megam.concurrent.Scheduler]].
 *
 * Waiters are stored as a [[org.megam.util.Promise.K]]. `K`s
 ...
 * compressed OOPS. See comments on [[org.megam.util.Promise.State]]
 * for details.
 */
