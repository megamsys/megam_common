 ...
 *
 */
import io.megam.concurrent.{Offer, Scheduler, Tx}
...
object Future {
   ...
   * It will catch nonfatal (see: [[io.megam.util.NonFatal]])
 ...
 *
 * Futures are also [[io.megam.util.Cancellable]], but with
...
abstract class Future[+A] extends Awaitable[A] {
  import io.megam.util.Future.DEFAULT_TIMEOUT
...
private[util] object FutureBenchmark {
  ...
  import io.megam.conversions.storage._
