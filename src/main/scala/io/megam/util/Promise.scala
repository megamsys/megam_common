...
import io.megam.concurrent.Scheduler
...
object Promise {
  ...
  sealed trait Responder[A] extends Future[A] {
     ...
     * {{io.megam.util.Monitor}} for details.
     */
...
/**
 * A writeable [[io.megam.util.Future]] that supports merging.
 * Callbacks (responders) of Promises are scheduled with
 * [[io.megam.concurrent.Scheduler]].
 *
 ...
 * [[io.megam.concurrent.Scheduler]].
 *
 * Waiters are stored as a [[io.megam.util.Promise.K]]. `K`s
 ...
 * compressed OOPS. See comments on [[io.megam.util.Promise.State]]
 * for details.
 */
