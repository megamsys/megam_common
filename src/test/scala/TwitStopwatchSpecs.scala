...
class TwitStopwatchSpecs extends Specification {
  ...
  def is =
    "TwitStopwatchSpecs".title ^ end ^
      """
  Twitter stop watch that starts and stops testing.
    """ ^ end ^
      "The Twit Stopwatch Should" ^
      "Correctly print the howlong time of a run" ! TwitWatch.succeeds ^
      end
